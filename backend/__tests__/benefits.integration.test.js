...
describe("Benefit Controller", () => {
  ...
  it("should create a new benefit", async () => {
    const response = await request(app).post("/benefits").send({
      object: "New Object",
      unit: UnitTypes.ITEM,
      price_per_unit: 10.5,
    });
  ...
  it("should update a benefit", async () => {
    ...
    const response = await request(app)
      ...
      .send({ object: "Updated Benefit", unit: 2, price_per_unit: 150.0 });
  ...
  it("should return 404 if benefit not found", async () => {
    const response = await request(app).delete("/benefits/999999");
